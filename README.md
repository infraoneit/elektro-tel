...
1. [Projekt Setup (Zero to Hero)](#2-projekt-setup)
1. [Technischer Stack & Design System](#3-technischer-stack)
1. [Das Content Management System (Keystatic)](#4-cms)
1. [Die Data Engine (cms.ts)](#5-data-engine)
1. [Frontend Architektur: Block Pattern](#6-block-pattern)
1. [Netlify Forms Integration (Deep Dive)](#7-netlify-forms)
1. [Deployment Pipeline](#8-deployment)
1. [Troubleshooting Guide](#9-troubleshooting)
...
1. **Schema:** Erweitern Sie `keystatic.config.ts` (im `pages` Schema).
...
1. **Mapping:** Fügen Sie ihn im `SectionRenderer` hinzu.
...
- **Fix:**
  ...
  1. Lokal `npm run build` ausführen. Wenn es lokal knallt, knallt es auch online.
